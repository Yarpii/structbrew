<section class="mx-auto w-[92%] sm:w-[90%] md:w-[88%] lg:w-[85%] py-7 md:py-10" x-data="{ showPass: false, showConfirm: false, loading: false }">
    ...
    <div class="space-y-4">
        <?php if (!empty($flashError)): ?>
            <div class="rounded-2xl border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm text-rose-700 dark:text-rose-200"><?= htmlspecialchars((string) $flashError) ?></div>
        <?php endif; ?>
        <?php if (!empty($flashSuccess)): ?>
            <div class="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-200"><?= htmlspecialchars((string) $flashSuccess) ?></div>
        <?php endif; ?>
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_0.8fr]">
            <div class="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 md:p-7" style="box-shadow: var(--shadow-sm)">
                ...
                <form class="space-y-4" method="POST" action="/register" @submit="loading = true">
                    ...
                    <?php if (!empty($supportsCustomerGroups) && !empty($customerGroups)): ?>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold text-[var(--color-text)]" for="reg-group">Account Type</label>
                            <select id="reg-group" name="customer_group" class="h-11 w-full rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var,--color-bg)] px-3.5 text-[var,--color-text)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var(--color-accent)]/10">
                                <?php foreach ($customerGroups as $groupKey => $group): ?>
                                    <option value="<?= htmlspecialchars((string) $groupKey) ?>" <?= (($defaultCustomerGroup ?? 'retail') === $groupKey) ? 'selected' : '' ?>><?= htmlspecialchars((string) $group['label']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <p class="mt-2 text-sm text-[var(--color-muted)]">Choose the type that best matches your relationship with the store.</p>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input id="reg-firstname" name="first_name" type="text" required placeholder="John" autocomplete="given-name"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var,--color-muted)]" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                                <input id="reg-lastname" name="last_name" type="text" required placeholder="Smith" autocomplete="family-name"
                                       class="h-11 w-full rounded-[var(--radius-input)] border border-[var(--color-border)] bg-[var,--color-bg)] pl-10 pr-3.5 text-[var,--color-text)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var,--color-accent)]/10">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="reg-email" name="email" type="email" required placeholder="name@example.com" autocomplete="email" spellcheck="false"
                                   class="h-11 w-full rounded-[var(--radius-input)] border border-[var,--color-border)] bg-[var,--color-bg)] pl-10 pr-3.5 text-[var,--color-text)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var,--color-accent)]/10">
                        </div>
                    </div>

                    <div>
                        <label class="mb-1.5 block text-sm font-semibold text-[var(--color-text)]" for="reg-dob">Date of Birth (optional)</label>
                        <input id="reg-dob" name="date_of_birth" type="date"
                               class="h-11 w-full rounded-[var(--radius-input)] border border-[var,--color-border)] bg-[var,--color-bg)] px-3.5 text-[var,--color-text)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var,--color-accent)]/10">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="reg-pass" name="password" :type="showPass ? 'text' : 'password'" required placeholder="Minimum 8 characters" autocomplete="new-password" minlength="8"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="reg-pass-confirm" name="password_confirmation" :type="showConfirm ? 'text' : 'password'" required placeholder="Repeat your password" autocomplete="new-password"
                                   class="h-11 w-full rounded-[var(--radius-input)] border border-[var,--color-border)] bg-[var,--color-bg)] pl-10 pr-11 text-[var,--color-text)] focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[var,--color-accent)]/10">
            ...
            <div class="rounded-2xl border border-[var(--color-border)] bg-[var,--color-surface)] p-5 md:p-7" style="box-shadow: var(--shadow-sm)">
                ...
                <ul class="space-y-2.5 text-sm">
                    ...
                    <li class="flex items-start gap-2.5 rounded-lg border border-[var,--color-border)] bg-[var,--color-bg)] px-3 py-2.5">
                        <svg class="shrink-0 mt-0.5" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                        <span class="text-[var,--color-text)]">Business and partner account options</span>
                    </li>
                    <li class="flex items-start gap-2.5 rounded-lg border border-[var,--color-border)] bg-[var,--color-bg)] px-3 py-2.5">
                        <svg class="shrink-0 mt-0.5" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
