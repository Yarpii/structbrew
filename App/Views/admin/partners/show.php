...
<div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
    <a href="/admin/partners" class="hover:text-gray-700">Partner Applications</a>
    <span>/</span>
    <span class="text-gray-900"><?= htmlspecialchars(($application['first_name'] ?? '') . ' ' . ($application['last_name'] ?? '')) ?></span>
</div>
...
<div class="grid gap-6 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="p-6 grid gap-4 sm:grid-cols-2">
                <div>
                    <p class="text-xs font-medium text-gray-400 uppercase mb-1">Name</p>
                    <p class="text-sm text-gray-900"><?= htmlspecialchars(($application['first_name'] ?? '') . ' ' . ($application['last_name'] ?? '')) ?></p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900"><?= htmlspecialchars($application['company'] ?? '—') ?></p>
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    <p class="text-xs font-medium text-gray-400 uppercase mb-1">Website / Channel</p>
        ...
        <div class="bg-green-50 rounded-xl border border-green-200 p-5">
            <p class="text-sm font-semibold text-green-800">Partner account created</p>
            <p class="text-xs text-green-700 mt-1">Referral code: <strong class="font-mono"><?= htmlspecialchars($partnerAccount['referral_code'] ?? '') ?></strong> — Commission: <?= number_format((float) ($partnerAccount['commission_rate'] ?? 10), 2) ?>%</p>
            <a href="/admin/partners/accounts/<?= (int) $partnerAccount['id'] ?>" class="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-green-700 hover:text-green-900">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <form method="POST" action="/admin/partners/<?= (int) $application['id'] ?>/approve">
                <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
                <div class="mb-3">
                    <label class="block text-xs font-medium text-gray-600 mb-1.5">Commission Rate (%)</label>
                    <input type="number" name="commission_rate" value="10" min="0" max="100" step="0.5"
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <form method="POST" action="/admin/partners/<?= (int) $application['id'] ?>/reject">
                <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
                <div class="mb-3">
