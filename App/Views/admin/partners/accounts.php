...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-4">
        <a href="/admin/partners" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Applications
        </a>
        <span class="text-gray-300">|</span>
        <span class="text-sm font-semibold text-gray-800">Partner Accounts</span>
    </div>
</div>

<!-- Search -->
<div class="bg-white rounded-xl border border-gray-200 mb-6">
    <form method="GET" action="/admin/partners/accounts" class="p-4 flex gap-4 items-end flex-wrap">
    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
                <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Partner</th>
                <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Ref Code</th>
                <th class="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase">Commission</th>
                <th class="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase">Clicks</th>
                <th class="text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase">Conversions</th>
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-medium text-gray-900"><?= htmlspecialchars(($acc['first_name'] ?? '') . ' ' . ($acc['last_name'] ?? '')) ?></p>
                            <p class="text-xs text-gray-400"><?= htmlspecialchars($acc['email'] ?? '') ?></p>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-3">
                    <span class="font-mono text-xs bg-gray-100 px-2 py-1 rounded"><?= htmlspecialchars($acc['referral_code'] ?? '') ?></span>
                </td>
                <td class="px-6 py-3 text-center text-gray-900"><?= number_format((float) ($acc['commission_rate'] ?? 0), 2) ?>%</td>
                <td class="px-6 py-3 text-center text-gray-900"><?= number_format((int) ($acc['total_clicks'] ?? 0)) ?></td>
                <td class="px-6 py-3 text-center text-gray-900"><?= number_format((int) ($acc['total_conversions'] ?? 0)) ?></td>
                <td class="px-6 py-3 text-right font-semibold text-gray-900">$<?= number_format((float) ($acc['balance'] ?? 0), 2) ?></td>
                ...
                <td class="px-6 py-3 text-right">
                    <a href="/admin/partners/accounts/<?= (int) $acc['id'] ?>" class="text-blue-600 hover:text-blue-700 text-xs font-medium">Manage</a>
                </td>
            </tr>
            <?php endforeach; else: ?>
            <tr><td colspan="8" class="px-6 py-12 text-center text-gray-400">No partner accounts found</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
