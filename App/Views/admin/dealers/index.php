...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-4">
        <div class="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
            <a href="/admin/dealers" class="px-4 py-2 font-medium <?= !isset($_GET['status']) || $_GET['status'] === '' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' ?>">All</a>
            <a href="/admin/dealers?status=pending" class="px-4 py-2 font-medium border-l border-gray-200 <?= ($_GET['status'] ?? '') === 'pending' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' ?>">
            ...
            <a href="/admin/dealers?status=approved" class="px-4 py-2 font-medium border-l border-gray-200 <?= ($_GET['status'] ?? '') === 'approved' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' ?>">Approved</a>
            <a href="/admin/dealers?status=rejected" class="px-4 py-2 font-medium border-l border-gray-200 <?= ($_GET['status'] ?? '') === 'rejected' ? 'bg-blue-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50' ?>">Rejected</a>
        </div>
        <a href="/admin/dealers/accounts" class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50">
            Dealer Accounts
...
<div class="bg-white rounded-xl border border-gray-200 mb-6">
    <form method="GET" action="/admin/dealers" class="p-4 flex gap-4 items-end flex-wrap">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-6 py-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-medium text-gray-900"><?= htmlspecialchars($app['company_name'] ?? '') ?></p>
                            <p class="text-xs text-gray-400"><?= htmlspecialchars($app['email'] ?? '') ?></p>
                        </div>
                    </div>
                </td>
                <td class="px-6 py-3 text-gray-700"><?= htmlspecialchars($app['contact_name'] ?? '—') ?></td>
                <td class="px-6 py-3 text-gray-500"><?= htmlspecialchars($app['country'] ?? '—') ?></td>
                <td class="px-6 py-3 text-gray-500 capitalize"><?= htmlspecialchars($app['business_type'] ?? '—') ?></td>
                ...
                <td class="px-6 py-3 text-right">
                    <a href="/admin/dealers/<?= (int) $app['id'] ?>" class="text-blue-600 hover:text-blue-700 text-xs font-medium">Review</a>
                </td>
            </tr>
            <?php endforeach; else: ?>
            <tr><td colspan="7" class="px-6 py-12 text-center text-gray-400">No applications found</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
...
<div class="mt-4 flex items-center justify-between text-sm text-gray-500">
    ...
    <div class="flex gap-2">
        <?php if ($applications['current_page'] > 1): ?>
            <a href="?page=<?= $applications['current_page'] - 1 ?><?= !empty($search) ? '&q=' . urlencode($search) : '' ?>" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">Previous</a>
        <?php endif; ?>
        <?php if ($applications['current_page'] < $applications['last_page']): ?>
            <a href="?page=<?= $applications['current_page'] + 1 ?><?= !empty($search) ? '&q=' . urlencode($search) : '' ?>" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">Next</a>
        <?php endif; ?>
    </div>
</div>
<?php endif; ?>
